...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload_photo'])) {
    ...
    for ($i = 0; $i < count($files['name']); $i++) {
        ...
        if ($mime && !in_array($mime, $allowedMimes)) {
            ...
            $filename = 'room_' . $roomId . '_' . uniqid() . '.' . $finalExt;
            ...
            if (!empty($customCaptions[$i])) {
                ...
                $caption = $category . ' - Room ' . $room['room_number'];
            }
            $stmt = $pdo->prepare('INSERT INTO room_photos (room_id, file_path, sort_order, caption) VALUES (?, ?, ?, ?)');
            ...
            $stmt->execute([$roomId, 'uploads/rooms/' . $filename, $sort, $caption]);
            $uploaded++;
        } else {
...
if (isset($_POST['update_caption'])) {
    ...
    $pdo->prepare('UPDATE room_photos SET caption = ? WHERE id = ? AND room_id = ?')->execute([$caption, $photoId, $roomId]);
...
if (isset($_POST['move_photo'])) {
    ...
    $stmt = $pdo->prepare('SELECT id, sort_order FROM room_photos WHERE id = ? AND room_id = ?');
    $stmt->execute([$photoId, $roomId]);
    $photo = $stmt->fetch();
    if ($photo && in_array($direction, ['up', 'down'])) {
        ...
        if ($newSort >= 0) {
            $swapStmt = $pdo->prepare('SELECT id FROM room_photos WHERE room_id = ? AND sort_order = ? AND id != ?');
            $swapStmt->execute([$roomId, $newSort, $photoId]);
            $swap = $swapStmt->fetchColumn();
            if ($swap) {
                $pdo->prepare('UPDATE room_photos SET sort_order = ? WHERE id = ?')->execute([$newSort, $photoId]);
                $pdo->prepare('UPDATE room_photos SET sort_order = ? WHERE id = ?')->execute([$currentSort, (int)$swap]);
            } else {
                $pdo->prepare('UPDATE room_photos SET sort_order = ? WHERE id = ?')->execute([$newSort, $photoId]);
            }
        }
    }
    header('Location: photos.php?room_id=' . $roomId);
    exit;
}
...
<?php if (!$photos): ?>
    <p>No photos uploaded yet. Upload at least 3-5 photos to showcase this room.</p>
<?php else: ?>
